	...
	else
	{
		...
		else if(*source == '/')
		{
			...
			if(plen == IS_COPY)
			{
				...
				if(*target) {if(!isDir(source) && isDir(target)) strcat(target, filename);}
				else
 #ifdef SWAP_KERNEL
...
*/
	...
	// last mounted game
	if(*_path == '_' || strrchr(_path, '/') == NULL) goto exit_mount;
	else
	{
...
patch:
	...
	//if(*_path && (strstr(_path, "/PS3_GAME/USRDIR/EBOOT.BIN")!=NULL)) _path[strlen(_path)-26] = NULL;
